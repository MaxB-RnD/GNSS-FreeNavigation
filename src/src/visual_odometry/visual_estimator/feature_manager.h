...
public:
  FeaturePerFrame(const Eigen::Matrix<double, 8, 1> &_point, double td)
      : z(0.0), is_used(false), parallax(0.0), dep_gradient(0.0) // Initialize members here
  {
    point.x() = _point(0);
    point.y() = _point(1);
    point.z() = _point(2);
  ...
  Vector2d uv;
  Vector2d velocity;
  double z;        // Initialize to 0.0
  bool is_used;    // Initialize to false
  double parallax; // Initialize to 0.0
  MatrixXd A;
  VectorXd b;
  double dep_gradient; // Initialize to 0.0
  double depth;        // lidar depth, initialized with _point(7)
};
